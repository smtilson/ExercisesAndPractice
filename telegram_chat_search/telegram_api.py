...
URL_PATTERN = r"/https?:\/\/[\w\-\.]+\.[a-z]{2,}(\/\S*)?/i"


LOG_FILE = "telegram_name_search_results.log"
NEW_LOG_FILE = "telegram_switch_code.log"


async def list_chats():
    client = TelegramClient("session_name", API_ID, API_ID)
...
async def test(chat_id, api_id, api_hash):
    # Create the client
...
async def search(chat_id, pattern, size=1000, offset=None, batches=5):
    ...
    with open(NEW_LOG_FILE, "a") as log:
        log.write(
        ...
        while batch_number < batches:
            print(
                f"Fetching messages {total_messages_checked+1}-{total_messages_checked+size}"
            ...
            batch_name_messages = []
            for msg in messages:
                if msg.text and re.match(pattern, msg.text.strip()):
    ...
    print(f"Results have been saved to {NEW_LOG_FILE}")
...
if __name__ == "__main__":
    # asyncio.run(test(chat_id=PHONE_NUMBER, api_id=API_ID, api_hash=API_ID))
