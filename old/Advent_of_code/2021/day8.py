...
def everything(data):
    data=convert(data)
    data = for_line_in_data(data, length_check)
    data = for_line_in_data(data, remove_finished)
    checks=[4,7,6]
    line=data[0]
    for num in checks:
        data = deduction_removal(data, num)
    Total=0
    for line in data:
        #print(line['possibilities'])
        decoded_message=decode(line)
        print(decoded_message)
        Total+=int(decoded_message)
    print(Total)

def convert_message_to_number(set_num):
    ans=''.join(set_num)
    return ans

def deduction_removal(data,n):
    for index, line in enumerate(data):
        line = deduce_single_line(line,n)
        line = remove_finished(line)
        data[index]=line
    return data

def for_line_in_data(data, function):
    for line in data:
        function(line)
    return data
...
def length_check(line):
    ...
    for clue in clues:
        if len(clue)==2:
            possibilities[clue] = {'1'}
        elif len(clue)== 3:
            possibilities[clue] = {'7'}
        elif len(clue)== 4:
            possibilities[clue] = {'4'}
        elif len(clue)== 5:
            possibilities[clue] = {'2','3','5'}
        elif len(clue)== 6:
            possibilities[clue] = {'0','6','9'}
        elif len(clue)== 7:
            possibilities[clue] = {'8'}
    line['possibilities'] = possibilities
    return line
...
def remove_finished(data):
    #solved={ for possibility ib data['possibilities'] if len(data['possibilites']['clue'])==1}
    if not 'solved_words' in data.keys():
        solved_words={}
        data['solved_words']=solved_words
    else:
        solved_words=data['solved_words']
    possibilities = data['possibilities']
    for clue, candidates in possibilities.items():
        if len(candidates)==1:
            solved_words[clue]=candidates
    removed_clues=set(())
    for clue in solved_words.keys():
        clue_set = convert_word(clue)
        #matches = [word in possibilities.keys() if len(word) == len(clue) ]
        for clue_poss in possibilities.keys():
            if convert_word(clue_poss) == clue_set:
                removed_clues.add(clue_poss)
    for clue in removed_clues:
        data['possibilities'].pop(clue)
    #print(f'removing {removed_clues}')
    return data

def contained_nums(n):
    if n==1:
        return {'0','3','9'}
    elif n==4:
        return {'9'}
...
def deduce_single_line(line, n):
    if n==6:
        return deduce_six(line)
    possibilities=line['possibilities']
    solved_words=line['solved_words']
    str_n= str(n)
    #print(f'testing{str_n}')
    nss = {word for word in solved_words.keys() if solved_words[word]=={str_n}}
    #print(f'the ones are:{ones}')
    for word in possibilities.keys():
        word_set =convert_word(word)
        #print(f'word={word}, word_set={word_set}')
        for test_word in nss:
            #print(f'test_word={test_word}, set={convert_word(test_word)}')
            #input()
            if convert_word(test_word).issubset(word_set):
                intersection=possibilities[word].intersection(contained_nums(n))
                possibilities[word]=intersection
                #print(f'the intersection is {intersection}')
                #print(f'the possibilities for {word} are now {possibilities[word]}')
            else:
                intersection = possibilities[word].intersection(not_contained_nums(n))
                possibilities[word] = intersection
    return line
...
def deduce_six(line):
    ...
    solved_words=line['solved_words']
    str_n= '6'
    sixes = {word for word in solved_words.keys() if solved_words[word]=={str_n}}
    #print(f'the ones are:{ones}')
    for word in possibilities.keys():
        word_set = convert_word(word)
        #print(f'word={word}, word_set={word_set}')
        for test_word in sixes:
            #print(f'test_word={test_word}, set={convert_word(test_word)}')
            #input()
            if convert_word(test_word).issuperset(word_set):
                intersection=possibilities[word].intersection({'5'})
                possibilities[word]=intersection
                #print(f'the intersection is {intersection}')
                #print(f'the possibilities for {word} are now {possibilities[word]}')
            else:
                intersection = possibilities[word].intersection({'2'})
                possibilities[word] = intersection
    return line
...
def decode(line):
    ...
    for word in message:
        word_set=convert_word(word)
        for solution in solved_words:
            if convert_word(solution)==word_set:
                #print(solved_words[solution])
                answer=convert_message_to_number(solved_words[solution])
                decoded_message += str(answer)
